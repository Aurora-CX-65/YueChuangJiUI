<template>
  <div class="admin-book-reviews">
    <el-card shadow="never">
      <div class="header">
        <div class="title">书籍审核</div>
        <el-space>
  ...
  methods: {
    ...
    async loadPending() {
      ...
      try {
        ...
        if (this.isEditorOnly()) {
          res = await AdminService.getEditorReviewItems(this.page, this.size, 'book', this.status)
        } else {
          res = await AdminService.getReviewItems(this.page, this.size, 'book', this.status)
        }
    ...
    async loadHistory() {
      ...
      try {
        let res
        if (this.isEditorOnly()) {
          res = await AdminService.getEditorReviewHistory(1, 10, 'book')
        } else {
          res = await AdminService.getReviewHistory(1, 10, 'book')
        }
    ...
    async confirmApprove() {
      ...
      try {
        if (this.isEditorOnly()) {
          await AdminService.approveEditorBookReview(this.actionRow.targetId, this.approveComment.trim())
        } else {
    ...
    async confirmReject() {
      ...
      try {
        if (this.isEditorOnly()) {
          await AdminService.rejectEditorBookReview(this.actionRow.targetId, comment)
        } else {
