<template>
  <div class="admin-comment-reviews">
    <el-card shadow="never">
      <div class="header">
        <div class="title">评论审核</div>
        <el-space>
          <el-select v-model="status" placeholder="状态" size="small" style="width:140px" @change="loadPending">
      ...
      <div class="content">
        ...
        <el-empty v-else-if="items.length === 0" description="暂无评论审核数据" />
        <div class="table-wrapper" v-else>
          <el-table :data="items" border size="small" @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="55" />
            <el-table-column prop="targetId" label="评论ID" width="100" />
            <el-table-column prop="targetContent" label="内容" min-width="300" />
            ...
            <el-table-column label="操作" width="320" fixed="right">
              <template #default="{ row }">
                <el-space wrap v-if="row.status === 'pending' || status === 'pending'">
        ...
        <el-table v-else :data="history" size="small" border>
          <el-table-column prop="targetId" label="评论ID" width="100" />
          <el-table-column prop="targetContent" label="内容" />
    ...
    <el-dialog v-model="approveVisible" title="审核通过" width="520px">
      <el-form label-width="90px">
        <el-form-item label="审核意见">
          <el-input v-model="approveComment" type="textarea" :rows="3" placeholder="可选，填写审核通过意见" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="approveVisible = false">取消</el-button>
        <el-button type="primary" :loading="actionLoading" @click="confirmApprove">通过</el-button>
      </template>
    </el-dialog>

    <el-dialog v-model="rejectVisible" title="审核拒绝" width="520px">
      <el-form label-width="90px">
        <el-form-item label="审核意见">
          <el-input v-model="rejectComment" type="textarea" :rows="3" placeholder="请填写审核拒绝意见" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="rejectVisible = false">取消</el-button>
        <el-button type="danger" :loading="actionLoading" @click="confirmReject">拒绝</el-button>
      </template>
    </el-dialog>
  ...
  methods: {
    ...
    async loadPending() {
      ...
      try {
        ...
        if (this.isEditorOnly()) {
          res = await AdminService.getEditorReviewItems(this.page, this.size, 'comment', this.status)
        } else {
          res = await AdminService.getReviewItems(this.page, this.size, 'comment', this.status)
        }
        this.items = res?.records || []
        this.total = res?.total || 0
      } catch (e) {
        console.error('加载评论审核列表失败:', e)
      } finally {
        this.loading = false
      }
    },
    ...
    async loadHistory() {
      ...
      try {
        let res
        if (this.isEditorOnly()) {
          res = await AdminService.getEditorReviewHistory(1, 10, 'comment')
        } else {
          res = await AdminService.getReviewHistory(1, 10, 'comment')
        }
        this.history = res?.records || []
      } catch (e) {
        console.error('加载评论审核历史失败:', e)
      } finally {
    ...
    async confirmApprove() {
      ...
      try {
        if (this.isEditorOnly()) {
          await AdminService.approveEditorReviewItem(this.actionRow.id, this.approveComment.trim())
        } else {
          await AdminService.approveReviewItem(this.actionRow.id, this.approveComment.trim())
        }
    ...
    async confirmReject() {
      ...
      try {
        if (this.isEditorOnly()) {
          await AdminService.rejectEditorReviewItem(this.actionRow.id, comment)
        } else {
          await AdminService.rejectReviewItem(this.actionRow.id, comment)
        }
